def give_bmi(height: list[int | float], weight: list[int | float]) -> list[int | float]:
    ...
    """
    if not (isinstance(height, list) and all(isinstance(x, int) or isinstance(x, float) for x in height)):
        raise ValueError("Error: height must be a list of int or float")

    if not (isinstance(weight, list) and all(isinstance(x, int) or isinstance(x, float) for x in weight)):
    ...
    if len(height) != len(weight) or len(height) == 0 or len(weight) == 0:
        raise ValueError("Error: length of weight and height must be the same, and cannot be empty list")
...
def apply_limit(bmi: list[int | float], limit: int) -> list[bool]:
    ...
    """
    if not (isinstance(bmi, list) and all(isinstance(x, int) or isinstance(x, float) for x in bmi)):
